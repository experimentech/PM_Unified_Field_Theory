...
def fit_disk_halo_rotation_curve(
	...
	if best is not None:
		for _ in range(n_refine):
			disk_p = DiskParams(
				M_d=clamp(disk_bounds, 'M_d', perturb(best['disk'].M_d, 0.35)),
				R_d=clamp(disk_bounds, 'R_d', perturb(best['disk'].R_d, 0.35))
			)
			if isinstance(best['halo'], NFWParams):
				halo_p = NFWParams(
					rho_s=clamp(halo_bounds, 'rho_s', perturb(best['halo'].rho_s, 0.4)),
					r_s=clamp(halo_bounds, 'r_s', perturb(best['halo'].r_s, 0.4))
				)
			else:
				halo_p = BurkertParams(
					rho_0=clamp(halo_bounds, 'rho_0', perturb(best['halo'].rho_0, 0.4)),
					r_c=clamp(halo_bounds, 'r_c', perturb(best['halo'].r_c, 0.4))
				)
